...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setupLogo(userRol, logo);
    await cargarFiltrosDinamicos();
    restaurarFiltrosExtendidos();
    cargarLocalizaciones(urlParams.get('localizacion') || storedSearch.localizacion);
    sincronizarTipo(urlParams.get('tipo') || storedSearch.tipo, tipoBotones, tipoSelectExtendido, form);
    manejarTipoSeleccionado(tipoBotones, tipoSelectExtendido, form);

    if (form) {
        form.addEventListener('submit', e => manejarSubmit(e, form, resultsContainer, feedback));
    }

    if ([...urlParams.entries()].length > 0) {
        const params = Object.fromEntries(urlParams.entries());
        await ejecutarBusquedaConFiltros(params, resultsContainer);
    }
});

function setupLogo(rol, logo) {
    if (!logo) return;
    logo.href = rol === 'ADMIN' ? '../templates/index-admin.html' : '/index.html';
    document.querySelector('.btn-create')?.classList.toggle('d-none', rol !== 'ADMIN');
}

function sincronizarTipo(tipo, botones, selectExtendido, form) {
    if (!tipo) return;

    let tipoInput = form.querySelector('input[name="tipo"]') || document.createElement('input');
    tipoInput.type = 'hidden';
    tipoInput.name = 'tipo';
    tipoInput.value = tipo;
    if (!form.contains(tipoInput)) form.appendChild(tipoInput);

    botones.forEach(b => b.classList.toggle('selected', b.dataset.value === tipo));
    if (selectExtendido) selectExtendido.value = tipo;
}

function manejarTipoSeleccionado(botones, selectExtendido, form) {
    ...
    botones.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (selectExtendido) selectExtendido.value = btn.dataset.value;
            guardarEnStorage('tipo', btn.dataset.value);
        });
    });

    if (selectExtendido) {
        selectExtendido.addEventListener('change', () => {
            const valor = selectExtendido.value;
            tipoInput.value = valor;
            botones.forEach(b => b.classList.toggle('selected', b.dataset.value === valor));
            guardarEnStorage('tipo', valor);
        });
    }
}
...
async function manejarSubmit(e, form, resultsContainer, feedback) {
    e.preventDefault();
    feedback.textContent = '';
    ...
    const fInicio = new Date(datos.fechaInicio);
    const fFin = new Date(datos.fechaFin);

    if (!datos.fechaInicio || !datos.fechaFin) {
        return mostrarError('⚠️ Debes seleccionar una fecha de recogida y una de devolución.', resultsContainer);
    }

    if (fInicio < hoy) {
        return mostrarError('⚠️ La fecha de recogida no puede ser anterior a hoy.', resultsContainer);
    }

    if (fFin <= fInicio) {
        return mostrarError('⚠️ La fecha de devolución debe ser posterior a la de recogida.', resultsContainer);
    }

    if (datos.precioMax === '') delete datos.precioMax;

    localStorage.setItem('lastSearch', JSON.stringify(datos));
    history.replaceState(null, '', `?${new URLSearchParams(datos).toString()}`);

    await ejecutarBusquedaConFiltros(datos, resultsContainer);
}
...
async function ejecutarBusquedaConFiltros(params, resultsContainer) {
    resultsContainer.innerHTML = '<p class="text-muted">Buscando vehículos disponibles...</p>';
    try {
        const vehicles = await searchVehicles(params);
        renderResults(vehicles, params, resultsContainer);
    } catch (error) {
        mostrarError(`Error: ${error.message}`, resultsContainer);
    }
}

function renderResults(vehicles, filtros, container) {
    const userRol = localStorage.getItem('userRol');
    const fechaInicio = filtros?.fechaInicio || '';
    const fechaFin = filtros?.fechaFin || '';
    const tipo = filtros?.tipo || 'COCHE';
    const precioMax = parseFloat(filtros.precioMax || Infinity);
    const nplazasFiltro = parseInt(filtros.nplazas);
    ...
    const filtrados = vehicles.filter(vehicle => {
        ...
        return !isNaN(precio) &&
            ...
            (!isNaN(nplazasFiltro) ? plazas === nplazasFiltro : true);
    });

    if (filtrados.length === 0) {
        container.innerHTML = '<p class="text-muted">No hay vehículos disponibles con esos criterios.</p>';
        return;
    }

    container.innerHTML = filtrados.map(vehicle => {
        const imagenUrl = vehicle.imagenUrl?.trim();
        const qs = `matricula=${encodeURIComponent(vehicle.matricula)}&fechaInicio=${encodeURIComponent(fechaInicio)}&fechaFin=${encodeURIComponent(fechaFin)}&tipo=${encodeURIComponent(tipo)}`;
        ...
        return `
            <a href="../templates/car-detail.html?${qs}" style="text-decoration: none;">
                <div class="card mb-3 text-dark">
                    <div class="row g-0">
                        <div class="col-md-4">
                            <img src="${imagenUrl}" class="img-fluid rounded-start" alt="Imagen de ${vehicle.marca} ${vehicle.modelo}">
...
function restaurarFiltrosExtendidos() {
    ...
    selects.forEach(select => {
        if (stored[select.name]) {
            select.value = stored[select.name];
        }
    });
}
...
window.applyExtendedFilters = () => {
    ...
    document.querySelectorAll('#filter-menu select').forEach(select => {
        if (!form.querySelector(`[name="${select.name}"]`)) {
            const hiddenInput = document.createElement('input');
            hiddenInput.type = 'hidden';
            hiddenInput.name = select.name;
            form.appendChild(hiddenInput);
        ...
        select.addEventListener('change', () => {
            form.querySelector(`[name="${select.name}"]`).value = select.value;
        });
    });
};
